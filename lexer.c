	...
	while ( 1 )
	{
		...
		else if ( isdigit(str[token->nNextPos]) )
		{
			...
			if ( str[token->nNextPos - 1] == '.' )
			{
				...
				--token->nNextPos;
				token->Type = T_NUMBER;
				strncpy(token->str, strToken, TOKEN_SIZE);
...
TokenTypeEnum GetNextTokenPostfix(const char *str, Token *token)
{
	int i;
	char strToken[TOKEN_SIZE + 1];
